...
def parse_args(argv):
    ...
    parser.add_argument(
        "--hopfield", type=bool, default=False, help="Hopfield layers"
    )
    parser.add_argument(
...
def main(argv):
    ...
    if args.cuda:
        ...
        device = 'cpu'
    print('hopfield: ',args.hopfield)
    if args.hopfield: net = TCM(config=[2,2,2,2,2,2], head_dim=[8, 16, 32, 32, 16, 8], drop_path_rate=0.0, N=args.N, M=320, hopfield=True)
    else: net = TCM(config=[2,2,2,2,2,2], head_dim=[8, 16, 32, 32, 16, 8], drop_path_rate=0.0, N=args.N, M=320)
    net = net.to(device)
    net.eval()
    count = 0
    PSNR = 0
