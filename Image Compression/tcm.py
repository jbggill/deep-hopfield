...
class HopLayerLayer(nn.Module):
    def __init__(self, input_dim, hidden_size, output_dim, channels=3,num_heads= None,scaling=None):
        super(HopLayerLayer, self).__init__()
        self.input_dim = input_dim
        self.channels = channels        
        self.hopfield = HopfieldLayer(input_size=input_dim, hidden_size=hidden_size, output_size= output_dim,scaling=scaling)
        self.flatten = nn.Flatten()
...
class Block(nn.Module):
    def __init__(self, input_dim, output_dim, head_dim, window_size, drop_path, type='W', input_resolution=None,hopfield=False):
        """ 
        ...
        #self.msa = HopLayer(input_dim, 1024,output_dim)
            ...
            #pself.msa = HopLayerLayer(input_dim, 1024,output_dim)
...
class ConvTransBlock(nn.Module):
    def __init__(self, conv_dim, trans_dim, head_dim, window_size, drop_path, type='W',hopfield=False):
        ...
        self.trans_block = Block(self.trans_dim, self.trans_dim, self.head_dim, self.window_size, self.drop_path, self.type,hopfield=self.hopfield)
...
class SWAtten(AttentionBlock):
    def __init__(self, input_dim, output_dim, head_dim, window_size, drop_path, inter_dim=192,hopfield=False) -> None:
        if inter_dim is not None:
            super().__init__(N=inter_dim)
            self.non_local_block = SwinBlock(inter_dim, inter_dim, head_dim, window_size, drop_path,hopfield=hopfield)
        else:
            super().__init__(N=input_dim)
            self.non_local_block = SwinBlock(input_dim, input_dim, head_dim, window_size, drop_path,hopfield=hopfield)
        if inter_dim is not None:
...
class SwinBlock(nn.Module):
    def __init__(self, input_dim, output_dim, head_dim, window_size, drop_path,hopfield) -> None:
        super().__init__()
        self.block_1 = Block(input_dim, output_dim, head_dim, window_size, drop_path, type='W',hopfield=hopfield)
        self.block_2 = Block(input_dim, output_dim, head_dim, window_size, drop_path, type='SW',hopfield=hopfield)
        self.window_size = window_size
...
class TCM(CompressionModel):
    def __init__(self, config=[2, 2, 2, 2, 2, 2], head_dim=[8, 16, 32, 32, 16, 8], drop_path_rate=0, N=128, M=320, num_slices=5, max_support_slices=5,hopfield=False ,**kwargs):
        ...
        begin = 0
        self.hopfield= hopfield
        self.m_down1 = [ConvTransBlock(dim, dim, self.head_dim[0], self.window_size, dpr[i+begin], 'W' if not i%2 else 'SW',hopfield=self.hopfield) 
                      for i in range(config[0])] + \
                      [ResidualBlockWithStride(2*N, 2*N, stride=2)]
        self.m_down2 = [ConvTransBlock(dim, dim, self.head_dim[1], self.window_size, dpr[i+begin], 'W' if not i%2 else 'SW',hopfield=self.hopfield)
                      for i in range(config[1])] + \
                      [ResidualBlockWithStride(2*N, 2*N, stride=2)]
        self.m_down3 = [ConvTransBlock(dim, dim, self.head_dim[2], self.window_size, dpr[i+begin], 'W' if not i%2 else 'SW',hopfield=self.hopfield)
                      for i in range(config[2])] + \
                      [conv3x3(2*N, M, stride=2)]

        self.m_up1 = [ConvTransBlock(dim, dim, self.head_dim[3], self.window_size, dpr[i+begin], 'W' if not i%2 else 'SW',hopfield=self.hopfield) 
                      for i in range(config[3])] + \
                      [ResidualBlockUpsample(2*N, 2*N, 2)]
        self.m_up2 = [ConvTransBlock(dim, dim, self.head_dim[4], self.window_size, dpr[i+begin], 'W' if not i%2 else 'SW',hopfield=self.hopfield) 
                      for i in range(config[4])] + \
                      [ResidualBlockUpsample(2*N, 2*N, 2)]
        self.m_up3 = [ConvTransBlock(dim, dim, self.head_dim[5], self.window_size, dpr[i+begin], 'W' if not i%2 else 'SW',hopfield=self.hopfield) 
        ...
        self.ha_down1 = [ConvTransBlock(N, N, 32, 4, 0, 'W' if not i%2 else 'SW',hopfield=self.hopfield) 
                      for i in range(config[0])] + \
        ...
        self.hs_up1 = [ConvTransBlock(N, N, 32, 4, 0, 'W' if not i%2 else 'SW',hopfield=self.hopfield) 
                      for i in range(config[3])] + \
        ...
        self.hs_up2 = [ConvTransBlock(N, N, 32, 4, 0, 'W' if not i%2 else 'SW',hopfield=self.hopfield) 
                      for i in range(config[3])] + \
                      [subpel_conv3x3(2*N, 320, 2)]
